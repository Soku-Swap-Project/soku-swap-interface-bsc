...
const SlippageToleranceSettings = ({ translateString }: SlippageToleranceSettingsModalProps) => {
  ...
  return (
    <StyledSlippageToleranceSettings>
      <Label>
        <Text style={{ fontWeight: 600, color: '#05489c' }}>{TranslateString(88, 'Slippage tolerance')}</Text>
        <QuestionHelper
      ...
      <Options className="slippage_input_boxes">
        <Flex mb={['8px', 0]} mr={[0, '8px']}>
          {predefinedValues.map(({ label, value: predefinedValue }) => {
            const handleClick = () => setValue(predefinedValue)

            return (
              <Option key={predefinedValue} className="slippage_input_boxes">
        ...
        <Flex alignItems="center">
          <Option>
            <Input
              ...
              className="modal_input_box"
              style={{ background: '#d8d8d8', color: '#05489c' }}
            />
          </Option>
          <Option color="#000">
            <Text color="#05489c" fontSize="18px">
              %
            </Text>
          </Option>
        </Flex>
