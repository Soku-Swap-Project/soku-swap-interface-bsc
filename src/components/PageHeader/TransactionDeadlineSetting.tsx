...
const TransactionDeadlineSetting = ({ translateString }: TransactionDeadlineSettingModalProps) => {
  ...
  return (
    <StyledTransactionDeadlineSetting>
      <Label>
        <Text style={{ fontWeight: 600, color: '#05489c' }}>{TranslateString(90, 'Transaction deadline')}</Text>
        <QuestionHelper
      ...
      <Field>
        <Input
          ...
          style={{ backgroundColor: '#d8d8d8', color: '#05489c' }}
          className="modal_input_box"
        />
        <Text color="#05489c">Minutes</Text>
      </Field>
      {error && (
