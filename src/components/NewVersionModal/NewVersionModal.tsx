...
const NewVersionModal = () => {
  ...
  const isNewVersionModalClosedByUser = JSON.parse(localStorage.getItem('isNewVersionModalClosedByUser') as string)

  const handleCloseModal = () => {
    localStorage.setItem('isNewVersionModalClosedByUser', 'true')
      ...
      {isModalVisible && !isNewVersionModalClosedByUser && (
        <div className="new-version-modal">
          ...
          <div className="new-version-modal__switching-instruction">
            For now, use the nav links to toggle to see a taste of V2 by clicking Swap
          </div>
          <button type="button" className="new-version-modal__button" onClick={handleCloseModal}>
            Got it
          </button>
        </div>
      )}
